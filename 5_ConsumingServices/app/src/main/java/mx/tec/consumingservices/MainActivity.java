...
public class MainActivity extends Activity {
           ...
           while (eventType != XmlPullParser.END_DOCUMENT) {
                if(eventType == XmlPullParser.START_DOCUMENT) {
                    ...
                    if(tag.equals("humidity")) {
                        ...
                        units = parser.getAttributeValue(null, "unit");
                        Log.i("I", "Parsing icon: " + units);
                    }

                    if(tag.equals("speed")) {
                        wind = parser.getAttributeValue(null, "name");
                        Log.i("I", "Parsing temperature: " + wind);
                    }
                    if(tag.equals("direction")) {
                        direction = parser.getAttributeValue(null, "name");
                        Log.i("I", "Parsing temperature: " + direction);
                    }
    ...
    public void sendRequest() {
        ...
        //https://api.openweathermap.org/data/2.5/weather?zip=76086,mx&appid=798624fafafa92dfd9bb91e6b33d9f67&units=metric&mode=xml
        String apiURL = "";
        String baseURL = "http://api.openweathermap.org/data/2.5/weather?";
        String responseMode ="&mode=xml";
        String country = ",mx";
        String zip = "zip=" + cityTxt;
        String units = "&units=metric";
        String apiKey="&APPID=";
        apiURL = baseURL +  zip + country + responseMode + units + apiKey;
